// Покращена логіка гри з проявленням від mask == 2, яке гарантовано зʼєднане з ігровим полем
...
function init() {
  ...
  function fillMask() {
  // Очищуємо попередні маски
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      if (mask[y][x] === 1 || mask[y][x] === 3) mask[y][x] = 0;
    }
  }

  // Перепризначаємо рамку як стартову зону (mask === 2)
  for (let y = 0; y < rows; y++) {
    mask[y][0] = 2;
    mask[y][cols - 1] = 2;
  }
